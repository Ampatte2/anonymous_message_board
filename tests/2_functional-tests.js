...
suite('Functional Tests', function() {
  ...
  suite('API ROUTING FOR /api/threads/:board', function() {
    ...
    suite('GET', function() {
      test("get array with 10 most recently bumped threads with 3 most recent replies", function(done){
        chai.request(server)
    ...
    suite('DELETE', function() {
      ...
      test("No delete with wrong delete_password", function(done){
        chai.request(server)
            .delete("/api/threads/testBoard/")
            .send({board: "testBoard",
  ...
  suite('API ROUTING FOR /api/replies/:board', function() {
    let testReplyId;
    suite('POST', function() {
      test("Create reply on any given thread with thread_id", function(done){
        chai.request(server)
    ...
    suite('DELETE', function() {
      test("No reply deleteing with board, thread_id, delete_password, wrong reply_id", function(done){
        chai.request(server)
            .delete("/api/replies/testBoard")
            .send({board: "testBoard",
                  thread_id: testThreadId2,
                  reply_id: "100000000000",
                  delete_password: "testPass"})
            .end((err, res)=>{
              assert.equal(res.status, 200);
